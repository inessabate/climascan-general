...
Incluye:
- Silenciado de logs (Spark + log4j) y resumen limpio de métricas (+CSV)
- Auditoría de etiqueta (cuantiles) segura (approxQuantile con floats)
- SMAPE como métrica adicional
- Target encoding sin leakage (CP y lob×segmento)
- GBT (log1p + corrección de sesgo lognormal)
- GLM Tweedie robusto (ε mínimo + winsor opcional + re-escalado de y + filtros finitos/nnz)
- GLM Gamma robusto (ε + winsor + re-escalado): intenta link log → link inverse → fallback lognormal
- Two-part (LR y>0 + severidad robusta) aplicando el modelo de severidad a TODO VALID (sin joins)
- Deciles con repartition y baselines
- Per-LOB: reusa hiperparámetros del GBT global (sin TVS por LOB) + baseline por LOB y BLEND λ=n/(n+α)
"""
...
USE_TWO_PART = True
USE_PER_LOB = True
USE_SCALER_FOR_GLM = False  # muy importante: GLM usará features_raw sin escalar


# Tuning Tweedie
TWEEDIE_VGRID = (1.3, 1.5, 1.7)

# Deciles
DECILES_REPARTITION = 200  # evita WindowExec single partition
...
# =====================
# Auditoría etiqueta (cuantiles seguros)
# =====================
...
def prepare_datasets(train, valid, prep_pipeline, use_scaler_for_glm=USE_SCALER_FOR_GLM):
    """
    - Fit del pipeline de imputación + OHE + assembler en TRAIN
    - Filtro de vectores no finitos/nnz
    - Si use_scaler_for_glm=True → añade StandardScaler para GLM; si no, GLM usa features_raw
    """
    ...
    train_prep = train_prep.filter(is_finite("features_raw")).filter(has_nnz("features_raw"))
    valid_prep = valid_prep.filter(is_finite("features_raw")).filter(has_nnz("features_raw"))

    if use_scaler_for_glm:
        ...
        # Seguridad extra para GLM
        train_prep = train_prep.filter(is_finite("features_glm")).filter(has_nnz("features_glm"))
        valid_prep = valid_prep.filter(is_finite("features_glm")).filter(has_nnz("features_glm"))
    else:
        # GLM trabajará sobre el vector sin escalar
        train_prep = train_prep.withColumn("features_glm", F.col("features_raw"))
        valid_prep = valid_prep.withColumn("features_glm", F.col("features_raw"))
...
def train_gbt_log(train_prep, valid_prep, tune=True, base_params=None, parallelism=1):
    """
    Si tune=True → TVS con parallelism controlado y devuelve best_params.
    Si tune=False → entrena un único GBT con base_params.
    """
    if tune:
...
def tune_tweedie(train_prep, valid_prep, vgrid=TWEEDIE_VGRID):
    """
    Tweedie robusto con escala:
      - y_tw = max(carga, ε)
      - y_tw_s = y_tw / scale  (scale = p99 de TRAIN)
      - GLM sobre y_tw_s; pred desescalada: pred * scale
    """
    ...
    tr = tr.withColumn("carga_tw_s", F.col("carga_tw")/F.lit(scale))
...
def train_glm_gamma(train_prep, valid_prep):
    """
    Gamma robusto con escala:
      - y_gm_s = y / scale  (scale = p99 de TRAIN positivo)
      - Intento Gamma(link=log) → Gamma(link=inverse) → Fallback lognormal sobre log(y/scale)
      - Desescalar predicciones
      Devuelve (modelo, pred_valid_pos, valid_pos, info_dict_con_scale_y_tipo)
    """
    ...
    gm_train = gm_train.withColumn("carga_gm_s", F.col("carga")/F.lit(scale))
    gm_valid = gm_valid.withColumn("carga_gm_s", F.col("carga")/F.lit(scale))
    ...
    # 3) Fallback: Lognormal (GLM gaussian sobre log(y/scale))
    ...
    try:
        ...
        print("[INFO] Usando fallback lognormal (GLM sobre log(y/scale)).")
...
def train_two_part(train_prep, valid_prep):
    """
    Clasificador y>0 + severidad robusta.
    Importante: aplicamos el modelo de severidad DIRECTAMENTE a todo VALID (no hacemos joins por ID).
    """
    # Clasificador de positividad
    ...
    # Severidad condicional (robusta) entrenada en positivos, aplicada a TODO VALID
    gm_model, sev_valid_pos, _, info = train_glm_gamma(train_prep, valid_prep)
    if info["type"].startswith("gamma"):
        sev_all = gm_model.transform(pv_valid).withColumn("sev_pred", F.col("pred_gm_s")*F.lit(info["scale"]))
    else:
        # lognormal: pred_log_s
        sev_all = gm_model.transform(pv_valid).withColumn("sev_pred", F.exp(F.col("pred_log_s"))*F.lit(info["scale"]))
...
def baseline_per_lob(train_lob, valid_lob):
    # baseline = media por segmento dentro del LOB
    base = baseline_mean(train_lob, valid_lob, ["segmento_cliente_detalle"])
    return base
...
def blend_predictions(pred_df, base_df, key_col="siniestro_hash", alpha=10000.0):
    ...
    return (pred_df.select(key_col, F.col("pred_carga").alias("pred_model"))
            .join(base_df.select(key_col, F.col("pred_carga").alias("pred_base")), key_col, "inner")
...
def train_gbt_per_lob(df_global_raw, lob_value, base_params, winsor_train=True, winsor_q=0.999, key_col="siniestro_hash"):
    """
    Entrena un GBT por LOB reutilizando hiperparámetros (sin TVS) para evitar multiprocessing.
    Evalúa baseline por LOB y BLEND λ=n/(n+α). Devuelve (rmse_mejor, mae_mejor).
    """
    ...
    # Asegura clave para el blend si no estuviera
    if key_col not in va.columns:
        va = va.withColumn("_row_id_", F.monotonically_increasing_id())
        key_col = "_row_id_"

    if "codigo_postal_norm" in df_lob.columns:
        tr, va = add_target_encoding(tr, va, ["codigo_postal_norm"], name="te_cp")
        if "te_cp_mean" in tr.columns:
            num_cols_lob = num_cols_lob + ["te_cp_mean"]
    tr, va = add_target_encoding(tr, va, ["segmento_cliente_detalle"], name="te_seg")
    if "te_seg_mean" in tr.columns:
        num_cols_lob = num_cols_lob + ["te_seg_mean"]

    # Opcional: winsor en TRAIN para estabilizar GBT per-LOB
    if winsor_train:
        try:
    ...
    _, tr_p, va_p = prepare_datasets(tr, va, prep_pipeline_lob, use_scaler_for_glm=USE_SCALER_FOR_GLM)
    ...
    # Baseline y blend (ambos contienen key_col)
    ...
    if key_col not in pred_lob.columns:
        # si se perdió la clave en el pipeline, la tomamos de va_p (mismo orden), y la añadimos
        from pyspark.sql.window import Window
        w = Window.orderBy(F.monotonically_increasing_id())
        pred_lob = pred_lob.withColumn("_tmp_id", F.row_number().over(w))
        va_p     = va_p.withColumn("_tmp_id", F.row_number().over(w))
        pred_lob = pred_lob.join(va_p.select("_tmp_id", key_col), on="_tmp_id", how="left").drop("_tmp_id")
...
def main():
    ...
    # 3) Auditoría de etiqueta
    quick_label_audit(train, "TRAIN raw")
    quick_label_audit(valid, "VALID raw")

    # 4) Target encoding sin leakage (CP y lob×segmento)
    ...
    num_cols += ["te_lob_seg_mean"]

    # 5) Preparación (fit en TRAIN)
    prep_pipeline = build_prep_pipeline(num_cols, cat_cols)
    prep_model, train_prep, valid_prep = prepare_datasets(train, valid, prep_pipeline, use_scaler_for_glm=USE_SCALER_FOR_GLM)

    # 6) Evaluadores y recolector de resultados
    evaluator_rmse = RegressionEvaluator(labelCol="carga", predictionCol="pred_carga", metricName="rmse")
    evaluator_mae  = RegressionEvaluator(labelCol="carga", predictionCol="pred_carga", metricName="mae")
    results = init_results()

    # 7) Modelos globales
    # 7.1 GBT GLOBAL (con tuning pero sin paralelismo agresivo)
    ...
    # 7.2 Tweedie (tuning robusto con escala)
    try:
    ...
    # 7.3 Gamma (>0) robusto con escala
    try:
    ...
    # 11) Resumen + CSV de métricas
    flush_results(results, spark, DIAG_DIR)

    # 12) Modelos per-LOB (reutilizando hiperparámetros)
    if USE_PER_LOB:
        lob_counts = (valid.groupBy("lob").count().orderBy(F.desc("count"))).collect()
        cand_lobs = [r["lob"] for r in lob_counts if r["count"] >= 2000]  # ajusta umbral
